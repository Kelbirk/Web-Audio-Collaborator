		...
		e.classList.remove("droptarget");
	  	...
	  	reader.onload = function (event) {
	  		...
	  		context.decodeAudioData( event.target.result, function(buffer) {
				...
				// Act on the result
				offlineContext.oncomplete = function(result) {
				  peaks = getPeaksAtThreshold(result.renderedBuffer.getChannelData(0), .1);
				  peaks = countIntervalsBetweenNearbyPeaks(peaks);
				  groupNeighborsByTempo(peaks);
...
function reverseBuffer( buffer ) {
	var newBuffer = context.createBuffer( buffer.numberOfChannels, buffer.length, buffer.sampleRate );
	if ( newBuffer ) {
...
Track.prototype.postLoadTasks = function() {
	...
	this.nameElement.innerHTML += " (" + this.buffer.duration.toFixed(1) + " sec " + bPM + " BPM)";
...
function groupNeighborsByTempo(intervalCounts) {
  ...
  for(var i = 0; i < tempoCounts.length; i++) {
    if(tempoCounts[i].count > currentHigh) {
      console.log(tempoCounts[i]);
      currentHigh = tempoCounts[i].count;
      bPM = tempoCounts[i].tempo;
    }
  }
}
