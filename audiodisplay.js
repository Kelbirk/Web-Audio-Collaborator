function drawBuffer( width, height, frame, buffer, color ) {
    ...
    for(var i=0; i < width; i++){
        ...
        frame.fillRect(i,(1+min)*amp,1,Math.max(1,(max-min)*amp));
    }
}
...
function drawRunningDisplay( frame, cache, centerInSeconds ) {
...
//        console.log("draw: " + leftEdgeIndex + ":" + rightEdgeIndex);
    for (var i = leftEdgeIndex; i<=rightEdgeIndex; i++) {
        frame.drawImage( cache[i], RUNNING_DISPLAY_HALF_WIDTH - center + (MAX_CANVAS_WIDTH*i), 0 );
//        if (center!=0)
